...
def summarize_cpue(species, biotic, timeframe):
    ...
    if biotic.lower() == 'individual':
        ...
        summarized = species_df.groupby(['Year', 'Month', 'Day']).agg(
            {'FishID': 'count', 'Length': 'mean', 'Site': 'unique'})
        summarized.rename(columns={"Length": "Catch"}, inplace=True)
    elif biotic == 'tot_len':
        # Sum the length of fish caught and assign it to 'Catch'
        summarized = species_df.groupby(['Year', 'Month', 'Day']).agg(
            {'FishID': 'count', 'Length': 'sum', 'Site': 'unique'})
        summarized.rename(columns={"Length": "Catch"}, inplace=True)
...
def summarize_water(abiotic, timeframe):
    ...
    if timeframe.lower() == 'year':
        ...
        print("ERROR: Invalid timeframe")
        return None  # Actually group the water data and pull the abiotic factor of interest
...
def abiotic_biotic_corr(species, biotic, abiotic, timeframe='year', lag_years=[0, 1, 2, 3, 4, 5, 10]):
    ...
    catch_per_unit_effort = summarize_cpue(species, biotic, timeframe)
    ...
    if timeframe.lower() == 'year':
        ...
        for lag in lag_years:
            col_name = "Abiotic -" + str(lag) + " years"
            lagged_df[col_name] = water_summary['Abiotic'].shift(periods=lag)

        joined_w_lag = pd.concat([joined, lagged_df], axis=1, join_axes=[joined.index])
        ...
        joined = catch_per_unit_effort.reset_index().join(water_summary, on=['Year', 'Month']).set_index(
            catch_per_unit_effort.index.names)
